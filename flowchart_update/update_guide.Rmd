...
If you have questions, feel free to contact me at rosecporta@icloud.com.
...
# read in past data
data_old <- read_csv(here::here("data", "flowchart_data_2022_corrected.csv"))
```
...
Note: If you identify new keywords in your process of adding the new data, make sure to add them in the chunk below.
```{r, eval=FALSE}
# age keywords
older_keywords <- c(
...
In the chunk below, you will need to fill in the years and the corresponding numbers of pages:
...
Run the code below to reformat the data and write separate csv files for each category. You can then upload each of these new csvs into google sheets to update the flowchart numbers.
```{r, eval=FALSE}
...
## Step 4: Create Summary Graphs

Run the code below to create a couple of barcharts summarizing color counts (g1) and percentages (g2) by gender and age.
```{r, eval=FALSE}
