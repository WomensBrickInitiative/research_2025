...
write.csv(heads_flesh_new, file = "heads_flesh_unique.csv")
...
heads_total <- read.csv("heads_total.csv")

heads_total <- heads_total[, -12] %>% 
  mutate(gender = case_when(str_detect(tolower(description), paste(male_keywords, collapse = "|"))
    ~ "male-coded",
     str_detect(tolower(description), paste(female_keywords, collapse = "|"))
    ~ "female-coded",
    TRUE ~ "no tag")) %>% 
  filter(str_detect(Name, "Sienna|Brown|Yellow|Nougat|Tan|Orange|Umber")) 

summary_total <- heads_total %>% group_by(Name, gender) %>% 
  summarize(total = n()) %>% 
  arrange(desc(total))

write.csv(summary_total, "summary_head.csv")

heads_total %>% filter(Name == "Reddish Brown") %>% 
  select(item_number, item_link, gender, category, description)

summary_total_others <- summary_total %>% 
  mutate(flesh_tone = case_when(Name == "Yellow" ~ "Yellow",
                                Name == "Light Nougat" ~ "Light Nougat",
                                TRUE ~ "Darker Tones")) %>% 
  group_by(flesh_tone, gender) %>% 
  summarise(count = sum(total))

summary_total_others %>% 
  ggplot(aes(x = factor(flesh_tone), y = count, fill = flesh_tone)) +
  geom_col(aes(group = gender), 
  ...
  scale_fill_manual(values = c("Yellow" = "#f3d000", 
                               "Light Nougat" = "#faccae",
                               "Darker Tones" = "#843419")) +
  labs(title = "Total Minfig Heads by Gender and Flesh Tone (2003 - 2024)",
       x = "",
       y = "Count of Minifig Heads", 
       fill = "Flesh Tone") +
...
```{r}
summary_total_wide <- summary_total %>% 
  select(Name, gender, total) %>% 
  pivot_wider(names_from = gender, values_from = total) 

totals <- colSums(summary_total_wide[, -1], na.rm = TRUE)
total_row <- data.frame(Name = "Total")
total_row[2:4] <- as.list(totals)
summary_total_final <- rbind(summary_total_wide, total_row)
  
export_table(summary_total_final, format = "html")
```
...
heads_total_year <- heads_total %>% group_by(category, gender) %>% 
  summarise(count = n()) %>% 
  mutate(total = sum(count))

heads_wide <- heads_total_year %>% 
  pivot_wider(names_from = gender, values_from = count)

heads_long <- heads_wide %>% 
  pivot_longer(cols = contains("al"), names_to = "name", 
                values_to = "count", values_drop_na = TRUE)

heads_long %>% 
  filter(name != "male-coded") %>% 
  ggplot(aes(y = count, x = factor(category), fill = name)) +
  geom_col(position = position_dodge(width = 1)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  scale_fill_wbi() +
  labs(title = "Total Heads vs. Total Female-coded Heads by Year (2003 - 2024)",
       x = "Year Released",
       y = "Count of Minifig Heads", 
       fill = "Category") 
```
```{r}
color_skin_summary <- heads_total %>%
  group_by(Name, category, ID, hexCode) %>%
  summarise(count = n()) %>% 
  filter(grepl("Reddish Brown|Tan|Nougat|Yellow|Sienna|Medium Brown", Name)) %>% 
  mutate(spectrum = case_when(Name == "Yellow" ~ 1,
                              Name == "Light Nougat" ~ 2,
                              Name == "Nougat" ~ 3,
                              Name == "Tan" ~ 4,
...
color_skin_sum_wide <- color_skin_summary %>% 
  pivot_wider(names_from = category, values_from = count, values_fill =0) %>% 
  mutate(total = sum(`2003`, `2004`, `2005`, `2006`, `2007`, `2008`, `2009`, `2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`, `2021`, `2022`, `2023`, `2024`))
```
...
update_heads <- color_skin_sum_wide %>% 
  ...
  labs(title = "Distribution of Flesh Tones of Minifigure Heads (2003-2024)",
       x = "Flesh Tones",
       y = "Count") +
...
color_skin_summary %>% 
  ...
  scale_fill_skintones() + 
  labs(title = "Distribution of Flesh Tones of Minifigure Heads by Year(2003-2024)",
       x = "Flesh Tones",
       y = "Count")
...
color_skin_summary %>% 
  ...
  scale_fill_skintones() + 
  labs(title = "Distribution of Flesh Tones of Minifigurine Heads by Year(2003-2024)",
       x = "Flesh Tones",
       y = "Count",
...
pie(x = color_skin_sum_wide$total, labels = color_skin_sum_wide$total, 
    main="Distribution of Flesh Tones of Minifigurine Heads (2003-2024)", 
...
color_skin_summary %>% 
  ...
  scale_fill_skintones() + 
  labs(title = "Trend of Flesh Tones of Minifigure Heads (2003-2024)",
       x = "Year",
       y = "Number of Heads",
       fill = "Flesh Tone")

color_skin_summary %>% 
  ggplot(aes(x = category, y = count, color = reorder(Name, -spectrum))) +
  geom_line(show.legend = TRUE, size = 1.2) +
  scale_color_skintones() +
  labs(title = "Distribution of Flesh Tones of Minifigurine Heads (2003-2024)",
       x = "Year",
       y = "Number of Heads",
       color = "Flesh Tone")

color_skin_sum_gender <- heads_total %>%
  group_by(gender, category) %>%
  summarise(count = n())

color_skin_sum_gender %>% 
  ggplot(aes(x = category, y = count, color = reorder(gender, -count))) +
  geom_line(show.legend = TRUE, size = 1.2) +
  scale_color_wbi() + 
  labs(title = "Trend of Minifigure Heads Gender (2003-2024)",
       x = "Year",
       y = "Number of Heads",
       color = "Gender")
...
heads_expression_all <- heads_total
pattern <- "(Annoyed|Evil|Frown|Happy|Sad|Scared|Sleepy|Smile|Smirk|Surprised|Sinister|Laugh|Neutral|Sneer|Fierce|Scowl|Asleep|Terrified|Firm|Worried|Grin|Confused|Barred|Grimace|Crooked|Eating|Disgust|Gritted|Stressed|Roaring|Wink|Licking|Blowing|Determined|Stern|Shocked)"

other_attr <- "(Vitiligo|Glasses|Goggles|Freckles|Hearing Aid|Mask|visor|Chin Strap|Beauty Mark|Tattoo|Facepaint)"

heads_expression_all <- heads_expression_all %>%
  mutate(
    matches = str_extract_all(description, pattern),  # list column
    expressionOne = sapply(matches, function(x) ifelse(length(x) >= 1, x[1], "none")),
    expressionTwo = sapply(matches, function(x) ifelse(length(x) >= 2, x[2], "none"))
  ) %>%
  select(-matches)

heads_expression_all <- heads_expression_all %>%
  mutate(
    matches = str_extract_all(description, other_attr),  # list column
    otherAttr = sapply(matches, function(x) ifelse(length(x) >= 1, x[1], "none"))
  ) %>%
  select(-matches)

heads_expression_all <- heads_expression_all %>% 
  mutate(expressionDivOne =case_when(grepl("Grin|Laugh|Happy|Smile", expressionOne) ~ "Happy", 
                                     grepl("Frown|Grimace|Angry|Gritted|Stressed|Roaring|Determined|Stern|Firm|Barred|Crooked|Scowl|Disgust|Annoyed|Sneer|Fierce", expressionOne) ~ "Angry", 
                                     grepl("Sad", expressionOne) ~ "Sad", 
                                     grepl("Neutral", expressionOne) ~ "Neutral", 
                                     grepl("Scared|Terrified|Worried", expressionOne) ~ "Scared", 
                                     grepl("Confused|Shocked|Surprised", expressionOne) ~ "Surprised",
                                     grepl("Sinister|Smirk|Evil", expressionOne) ~ "Evil", 
                                     grepl("none", expressionOne) ~ "NA", 
                                     TRUE ~ "others"),
         expressionDivTwo =case_when(grepl("Grin|Laugh|Happy|Smile", expressionTwo) ~ "Happy", 
                                     grepl("Frown|Grimace|Angry|Gritted|Stressed|Roaring|Determined|Stern|Firm|Barred|Crooked|Scowl|Disgust|Annoyed|Sneer|Fierce", expressionTwo) ~ "Angry", 
                                     grepl("Sad", expressionTwo) ~ "Sad", 
                                     grepl("Neutral", expressionTwo) ~ "Neutral", 
                                     grepl("Scared|Terrified|Worried", expressionTwo) ~ "Scared", 
                                     grepl("Confused|Shocked|Surprised", expressionTwo) ~ "Surprised",
                                     grepl("Sinister|Smirk|Evil", expressionTwo) ~ "Evil", 
                                     grepl("none", expressionTwo) ~ "NA", 
                                     TRUE ~ "others"))
write.csv(heads_expression_all, "heads_expression.csv")

head_expr_filtered <- heads_expression_all %>% 
  select(category, gender, Name, expressionDivOne, expressionDivTwo, otherAttr) %>% 
  filter((expressionDivOne != "NA"&expressionDivTwo != "NA")|otherAttr != "none") %>% 
  pivot_longer(cols = contains("expression"), names_to = "side", 
                values_to = "expression", values_drop_na = TRUE)

head_expr_gender <- head_expr_filtered %>% 
  group_by(gender, expression) %>% 
  summarise(count = n()) 

head_expr_gender %>% 
  filter(expression != "NA") %>% 
  ggplot(aes(x = reorder(expression, -count), y = count, fill = gender)) +
  geom_col(color = "black", linewidth = 0.1) +
  scale_fill_wbi() + 
  labs(title = "Distribution of Expression of Minifigure Heads by Gender(2003-2024)",
       x = "Expression",
       y = "Count", 
       fill = "Gender")

head_expr_flesh <- head_expr_filtered %>% 
  group_by(Name, expression) %>% 
  summarise(count = n()) 

head_expr_flesh %>% 
  filter(expression != "NA") %>% 
  ggplot(aes(x = reorder(expression, -count), y = count, fill = reorder(Name, -count))) +
  geom_col(color = "black", linewidth = 0.1) +
  scale_fill_skintones() + 
  labs(title = "Distribution of Expression of Minifigure Heads by Flesh Tones(2003-2024)",
       x = "Expression",
       y = "Count", 
       fill = "Flesh Tone")
```
